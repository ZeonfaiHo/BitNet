...
bitnet_lib_release = ctypes.CDLL('bitnet_kernels.release/libbitnet.so')
...
def bitnet_int8xint2_linear_release(input0, input1, s, ws, ret):
    out_shape = list(input0.shape)
    out_shape[-1] = input1.shape[0]
    ...
    bitnet_lib_release.bitlinear_int8xint2(*[ctypes.c_void_p(input0.data_ptr()), ctypes.c_void_p(input1.data_ptr()), ctypes.c_void_p(ret.data_ptr()), ctypes.c_void_p(s.data_ptr()), ctypes.c_void_p(ws.data_ptr()), ctypes.c_int(M), ctypes.c_int(N), ctypes.c_int(K), ctypes.c_void_p(stream.cuda_stream)])
...
if __name__ == '__main__':
    ...
    for N,K in test_list:
        ...
        weight_compressed = weight_repack(convert_weight_int8_to_int2_adsbrain(weight.to('cpu'))).to('cuda')
        weight_compressed_release = convert_weight_int8_to_int2(weight).to('cuda')
        ...
        for i in range(32):
            ...
            ret_release = torch.empty((1,N), dtype=torch.bfloat16, device=input0.device)
            out_release = bitnet_int8xint2_linear_release(input0, weight_compressed_release, s, ws, ret_release)
            
            print(f'custom == np {torch.all(out==out_np)}')
            print(f'release == np {torch.all(out_release==out_np)}')
            print(f'custom == release {torch.all(out==out_release)}')
        ...
        print(f'Shape{N,K}, W2A8: {time0.mean * 1e6:.2f}us, torch bf 16: {time1.mean * 1e6:.2f}us')
